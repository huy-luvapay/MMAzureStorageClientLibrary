...
-(void)execute
{
    ...
    // do-while (to allow for retries)
    {
        ...
        if(self.operationContext.isCancel == YES) {
            NSError *storageError = [NSError errorWithDomain:AZSErrorDomain code:AZSECancelRequest userInfo:@{NSLocalizedDescriptionKey: @"Request canceled"}];
        ...
        [task resume];
        self.storageCommand.sessionTask = task;
        //NSLog(@"**** sessionDataTasks addObject");
    }
}
...
//-(void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task needNewBodyStream:(void (^)(NSInputStream *))completionHandler;
 ...
 // TODO: Update status with data sent
     ...
     {
         self.operationContext.didSendBodyData(task, bytesSent, totalBytesSent, totalBytesExpectedToSend);
     }
