...
- (void)cancel {
    //NSLog(@"commands cancel: %lu", (unsigned long)self.commands.count);
    _isCancel = YES;
    for(int i = 0; i < self.commands.count; i++) {
        [self.commands[i].sessionTask cancel];
        //NSLog(@"self.commands[i].sessionTask: %@", self.commands[i].sessionTask.currentRequest.URL.absoluteString);
    }
